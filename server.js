...
async function runStartup() {
    ...
    CURRENT_VERSION = localVersion || "Unknown";
    console.log("\n━━━━━━━━ VERSION CHECK ━━━━━━━━");
    if (!remoteVersion) {
        console.log("Status : Failed To Check For Updates");
        ...
        if (localVersion !== remoteVersion) {
            ...
            const updatePrompt = readline.createInterface({
                input: process.stdin,
                output: process.stdout
            });
            const input = await new Promise(resolve => {
                updatePrompt.question("> ", answer => {
                    updatePrompt.close();
                    resolve(answer.trim());
                });
            });
        ...
        (portInput) => {
            ...
            rl.question(
                ...
                (choice) => {
                    ...
                    rl.close();
                    startServer(HOST, mode, CURRENT_VERSION);
                }
            );
        }
    );
}
function startServer(HOST, mode, version) {
  	console.clear();
  	printBanner();
  	log();
  	const app = express();
